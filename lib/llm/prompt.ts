...
- Each per-person roast must reference something specific from THEIR stats (their latency, their emoji habit, their message count percentage, their laugh count). Generic roasts are forbidden.
...
- Group verdict: 2-3 sentences, total under 280 chars. Capture the chat's overall vibe.
- Per-person roasts: ONE line each, under 60 chars. Sharp and specific.
...
export function buildUserPrompt(stats: PromptStats): string {
  const lines: string[] = [];
  lines.push(`GROUP: ${stats.group.total} messages over ${stats.group.daysActive} days. Peak hour: ${stats.group.peakHour}h.`);
  lines.push("");
    ...
    const ghost =
      p.longestGhostMin === null
        ...
        : p.longestGhostMin > 60
          ...
          : "";
    lines.push(
      `- ${p.name}: ${p.count} msgs (${p.percent.toFixed(0)}%), ${lat}, top emojis ${emojis}, ${p.laughs} laughs, avg ${Math.round(p.msgLenAvgChars)} chars${ghost}`,
    );
  }
  lines.push("");
...
export function fallbackVerdict(names: string[]): {
  group_verdict: string;
